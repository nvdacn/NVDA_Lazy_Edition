...
- #### 安装程序
  ...
  - 及时翻译插件被 Polyglot 插件替换；
  - 更新插件。

    - **注意：数字处理插件因与 NVDA 2026.1 不兼容而被禁用，可安装此版本，待后续该插件更新后，通过插件商店进行更新即可启用。如 NVDA 2026.2 发布后该插件仍未更新，其将会被从本程序中删除。**

- #### NVDA 配置恢复工具

  - 新增成功恢复配置后询问是否删除配置的对话框。
  - 修正未能正确启动/退出 NVDA 的错误；
  - 将配置备份的路径更改为 `"%UserProfile%\Documents\NVDA_Lazy_Edition_Backup"`。
    **注意：此前备份的配置无法使用新版本的配置恢复工具进行恢复。**
...
- #### 全局
  ...
  - 新增：[GitCode](https://gitcode.com/nvdacn/NVDA_Lazy_Edition) 发布渠道，以便于 GitHUB 渠道下载缓慢的用户使用。
...
- #### 安装程序
  ...
  - 更新 NVDA；
  - 若选择安装 WorldVoice 语音，则会将其设置为首次运行 NVDA 时默认的语音合成器，可在“选择附加任务”页面关闭该选项或选择其他语音；
  - 更新插件。
...
- #### 安装程序
  ...
  - 更新插件。
    ...
    - **注意：Access8Math插件因与 NVDA 2024.1 存在热键冲突，故在此版本被设置为手动安装，即在安装程序的“选择组件”页面，选择任意安装模式均不会自动安装，但在“自定义安装”类别可手动选择该插件的复选框进行安装。如 NVDA 2024.2 发布后该插件仍未更新，其将会被从本程序中删除。**
...
- #### 安装程序
  ...
  - 更新插件。
    ...
    - **注意：Access8Math、DragAndDrop、数字处理插件因与 NVDA 2024.1 存在热键冲突或不兼容，故在此版本被设置为手动安装，即在安装程序的“选择组件”页面，选择任意安装模式均不会自动安装，但在“自定义安装”类别可手动选择相应插件的复选框进行安装。如 NVDA 2024.2 发布后上述插件仍未更新，其将会被从本程序中删除。**
