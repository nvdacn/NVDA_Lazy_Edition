...
1. 音频管理器、百度翻译、DragAndDrop、数字处理、新翼OCR插件上不兼容 NVDA 2024.1；
2. 新增的 Access8Math 插件与 NVDA 2024.1存在热键冲突。

上述插件在此版本将被设置为手动安装，即在安装程序的“选择组件”页面，选择任意安装模式均不会自动安装，但在“自定义安装”类别可手动选择相应插件的复选框进行安装。
另需注意：不兼容的插件会在 NVDA 启动时被禁用。
待上述插件更新后，本程序将发布正式版本，如 NVDA 2024.2 发布后上述插件仍为更新，其将会被从本程序中删除。
如不需要上述插件，可放心安装此版本。
