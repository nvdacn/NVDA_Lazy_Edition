...
- Polyglot 插件：
  - 禁用所有需要安装依赖软件、填写密钥或账号信息后才能正常使用的翻译引擎，如需使用这些引擎，请根据插件帮助正确安装必需软件或填写相应信息后自行手动启用；
  - 启用“Google 翻译（免秘钥）”引擎的“使用镜像服务器 (translate.googleapis.mirror.nvdadr.com)”，使此引擎更加易用。
...
1. 运行本程序时，根据您的 NVDA 配置文件夹中是否存在 NVDA.ini 文件，会弹出不同的欢迎引导对话框，请根据提示选择所需的安装模式亦或者是否清空及是否备份您的现有配置，如选择备份您现有的 NVDA 配置，则备份的文件位于 `"%UserProfile%\Documents\NVDA_Lazy_Edition_Backup"` 文件夹下；
...
4. 通过本程序安装的 IBM ViaVoice TTS Runtime 语音引擎无法被自动删除，如需删除，请到程序和功能自行卸载；
